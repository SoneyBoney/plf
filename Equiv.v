...
Proof.
  ...
  - intros. apply E_IfFalse.
    * unfold bequiv in H. rewrite H. simpl. reflexivity.
    *  assumption. Qed.
...
Proof.
  intros. split.
  - intros. apply (while_true_nonterm b c st st') in H.
    apply H in H0. contradiction.
  - intros. inversion H0. subst. 
    + simpl in H5. discriminate.
    + subst. remember H as H'. 
    clear HeqH'.
    assert (H9: bequiv <{true}> <{true}>). 
    * unfold bequiv. intros. auto.
    * 
    apply (while_true_nonterm <{true}> <{ skip }> st'0 st') in H9.
    apply H9 in H7. contradiction. Qed.
...
Proof.
  ...
  - inversion H0; subst. 
    ...
    + rewrite H1 at 2. Search "E_Asgn". apply E_Asgn.
      rewrite <- H. auto.
  - inversion H0. subst. 
    assert (st = t_update st X (aeval st a)).
    + apply functional_extensionality.
      intros. rewrite <- H. rewrite t_update_same. auto.
...
Proof.
  ...
  - (* while *)
    ...
    + Search "while". Search (cequiv _ _ ). apply while_true. assumption.
...
Proof.
  ...
  - induction a1.
    induction n. auto; simpl; rewrite IHa2; auto.
    simpl; rewrite IHa2; auto.
    simpl; rewrite IHa2; auto.
    simpl; rewrite IHa2; auto.
    simpl; rewrite IHa2; auto.
    simpl; rewrite IHa2; auto.
  - induction a1. induction n; auto; simpl.
    + simpl; rewrite IHa2; auto.
    + simpl; rewrite IHa2; auto.
    + simpl; rewrite IHa2; auto.
    + simpl; rewrite IHa2; auto.
...
Proof.
  ...
  induction b; auto; simpl; repeat (rewrite <- optimize_0plus_aexp_sound); auto.
  - rewrite <- IHb; auto.
  - rewrite <- IHb1; rewrite <- IHb2; auto. Qed.
...
Proof.
  ...
  induction c; simpl; try apply refl_cequiv.
  ...
  - assert (bequiv b (optimize_0plus_bexp b)).
    ...
    destruct (optimize_0plus_bexp b) eqn:Heqb; simpl;
    try (apply CIf_congruence; assumption).
    + apply trans_cequiv with c1.
      * apply if_true. rewrite <- Heqb. apply optimize_0plus_bexp_sound.
      * auto.
    + apply trans_cequiv with c2.
      * apply if_false. rewrite <- Heqb. apply optimize_0plus_bexp_sound.
      * auto.
  - assert (bequiv b (optimize_0plus_bexp b)).
    { apply optimize_0plus_bexp_sound. }
    destruct (optimize_0plus_bexp b) eqn:Heqb; simpl;
    try (apply CWhile_congruence; assumption).
    + Search "while_true". apply (while_true b c H).
    + apply (while_false b c H). Qed.
...
Proof.
  unfold ctrans_sound. intros. unfold optimizer.
...
Proof.
  intros. induction H as [v1 | v2 | v3 | v4 | v5];
    try (simpl; rewrite IHvar_not_used_in_aexp1; rewrite IHvar_not_used_in_aexp2; auto).
  - simpl. auto.
  - simpl. apply t_update_neq. auto. Qed.
...
Proof.
  ...
  apply loop_never_stops with empty_st empty_st. unfold loop.
  apply Contra. apply E_Skip. Qed.
...
Proof.
  ...
  - inversion H2. inversion H5. subst.
    apply E_Seq with (st' := t_update st Y X0).
    * apply E_Havoc.
    * assert ((Y !-> X0; Imp.X !-> X; st) = (Imp.X !-> X;Y !-> X0;  st) ).
      { apply   t_update_permute. unfold not. intros. inversion H0. }
      rewrite H0. apply E_Havoc.
  - inversion H2. inversion H5. subst.
    apply E_Seq with (st' := t_update st Imp.X X0).
    * apply E_Havoc.
    * assert ((Imp.X !-> X0; Y !-> X; st) = (Y !-> X;Imp.X !-> X0;  st) ).
      { apply   t_update_permute. unfold not. intros. inversion H0. }
      rewrite  H0. apply E_Havoc. Qed.
...
Proof.
  ...
  remember (t_update (t_update empty_st X 1) Y 2) as st'.
  assert (empty_st =[ ptwice ]=> st' -> empty_st =[ pcopy ]=> st') by apply H.
  assert (~empty_st =[ pcopy ]=> st'). 
  { unfold not. intros.
    inversion H1. subst. inversion H7. subst. simpl in H8.
    assert (t_update (t_update empty_st X 1) Y 2 X = 1).
   { reflexivity. }
   assert (t_update (t_update empty_st X 1) Y 2 Y = 2).
   { reflexivity. }
   rewrite <- H8 in H2, H3. assert (st'0 X = 1).
   { apply H2. }
   assert (st'0 X = 2).
   { apply H3. }
    rewrite H5 in H6; discriminate.
  }
  apply H1. apply H0. unfold ptwice. subst. 
  apply E_Seq with (t_update empty_st X 1).
  - apply E_Havoc.
  - apply E_Havoc. Qed. 
...
Proof. 
  ...
  - subst. simpl in H0. apply negb_false_iff in H0. apply beq_nat_true in H0.
    apply H; assumption.
  - apply IHceval2.
    + rewrite H3 in H0_. inversion H0_. subst. inversion H8.  subst.
      simpl. rewrite t_update_eq. simpl. rewrite <- plus_n_Sm. intros. inversion H1.
    + assumption.  Qed. 
...
Proof.
  ...
  - subst. simpl in H0. apply negb_false_iff in H0. apply beq_nat_true in H0. apply H; assumption.
  - apply IHceval2. 
    + rewrite H3 in H0_. inversion H0_.  subst. simpl in H0. apply negb_true_iff in H0.
      apply beq_nat_false. assumption.
    + rewrite H2.  rewrite H3.  auto. Qed.
...
Proof. 
  split; intros.
  - inversion H; subst. apply E_WhileFalse. assumption.
    + exfalso. apply p1_may_diverge with st st'.
      * inversion H2. apply negb_true_iff in H1. apply beq_nat_false in H1. assumption.
      * assumption.
  - inversion H; subst. apply E_WhileFalse. assumption.
    + exfalso. apply p2_may_diverge with st st'.
      * inversion H2. apply negb_true_iff in H1. apply beq_nat_false in H1. assumption.
      * assumption. Qed.
...
Proof. 
  unfold not,cequiv. intros Contra.
  remember (t_update empty_st X 1) as st0_p3.
  remember (t_update st0_p3 Z 1) as st1_p3.
  remember (t_update st1_p3 X 0) as st2_p3.
  remember (t_update st2_p3 Z 9) as st3_p3.
  assert (st0_p3 =[ p3 ]=> st3_p3).
    { unfold p3. apply E_Seq with st1_p3. 
      - subst. apply E_Asgn. auto.
      - apply E_WhileTrue with st3_p3.
        + simpl. subst. simpl. auto.
        + apply E_Seq with st2_p3.
          * subst. apply E_Havoc.
          * subst. apply E_Havoc.
        + apply E_WhileFalse.
          * subst. simpl. auto. }
  remember (t_update (t_update empty_st X 0) Z 1) as p4_final.
  assert (Contra2: st3_p3 Z = p4_final Z).
    { rewrite Contra in H. inversion H. subst. inversion H2.  subst. inversion H5. subst.
      simpl in H5. simpl in H2. simpl in H6. simpl. subst.
      Search "t_update_shadow". auto. }
    subst. inversion Contra2. Qed.
...
Proof. 
  ...
  - remember p5. 
    ...
    + assert (st = t_update st X 1).
        { inversion H. apply negb_false_iff in H1. apply beq_nat_true in H1. 
          rewrite <- H1. symmetry. apply t_update_same. }
      rewrite H0 at 2. unfold p6. apply E_Asgn. auto.
    + inversion H0; subst. rename X into n.
      assert((t_update st X n) =[p6]=>st'').
  ...
  - inversion H; subst.
    ...
    + apply E_WhileTrue with (X !-> 1; st).
      * simpl. auto.
      * apply E_Havoc.
      * apply E_WhileFalse. simpl. auto.
    + assert (st = t_update st X 1).
      { symmetry. apply negb_false_iff in eqnat. apply beq_nat_true in eqnat. 
        rewrite <- eqnat. apply t_update_same. }
      rewrite <- H0.
      apply E_WhileFalse. auto. Qed.
...
Theorem swap_noninterfering_assignments: forall l1 l2 a1 a2,
  ...
  cequiv
    ...
    <{ l2 := a2; l1 := a1 }>.
Proof.
  intros. Search "cequiv".
  split; intros.
  - inversion H2; subst. inversion H5; subst. inversion H8; subst. 
    assert (aeval (l1 !-> aeval st a1; st) a2 = aeval st a2).
  ...
  - inversion H2; subst. inversion H5; subst. inversion H8; subst. 
    ...
    + apply E_Asgn. auto.
    + apply E_Asgn. apply aeval_weakening. auto. Qed.
...
Proof. 
  split; unfold capprox, not, c3,c4 ; intros.
  - assert ( empty_st =[ X := 2 ]=> (X !-> 2)).
      { apply E_Asgn. auto. }
    remember X.
    assert (empty_st = (s !-> 2) ).
    { Search "ceval_deterministic". apply ceval_deterministic with <{s := 2 }> empty_st.
      + apply H. apply E_Skip.
      + apply E_Asgn. auto. }
    assert (empty_st s = (s !-> 2) s).
    {  apply equal_f. auto. }
    unfold empty_st in H2. rewrite t_apply_empty in H2.
    rewrite t_update_eq in H2. inversion H2.
  - remember X.
    assert ( (s !-> 2)  = empty_st ).
    { Search "ceval_deterministic". apply ceval_deterministic with <{ skip }> empty_st.
      + apply H. apply E_Asgn. auto.
      + apply E_Skip. }
    assert (empty_st s = (s !-> 2) s).
    {  apply equal_f. auto. }
    unfold empty_st in H1. rewrite t_apply_empty in H1.
    rewrite t_update_eq in H1. inversion H1. Qed.
...
Proof.
  ...
  Search "loop_never_stops".
  - apply loop_never_stops in H. exfalso. assumption. Qed.
...
Proof.
  unfold zprop, capprox. intros. apply H0. apply H. Qed.
