...
Proof.
  unfold hoare_triple. intros.
  apply H in H1. exfalso. auto. Qed.
(** [] *)
...
Example hoare_asgn_examples1 :
  exists P,
    ...
    {{ X <= 10 }}.
Proof.
  exists ((X <= 10) [X |-> 2 * X]). apply hoare_asgn. Qed.
(** [] *)
...
Example hoare_asgn_examples2 :
  exists P,
    ...
    {{ 0 <=  X /\ X <= 5 }}.
Proof. 
  exists ((0 <=  X /\ X <= 5) [X |-> 3]). apply hoare_asgn. Qed.
...
Proof.
  ...
  split. inversion H0.
  - inversion H. subst. 
    assert ((X !-> st X; X !-> aeval st a; st) = (X !-> st X; st)).
    { Search "t_update_shadow".  apply t_update_shadow.   }
    rewrite H2. rewrite t_update_same. assumption.
  - inversion H0. subst. inversion H. subst. rewrite t_update_shadow.
    induction a; eauto.
    + simpl. rewrite t_update_same. apply t_update_eq.
    + simpl. rewrite t_update_same. rewrite t_update_eq. auto.
    + simpl. rewrite t_update_same. rewrite t_update_eq. auto.
    + simpl. rewrite t_update_same. rewrite t_update_eq. auto. Qed.
...
Proof.
  ...
  - inversion H. subst. 
    assert ((X !-> st X; X !-> aeval st a; st) = (X !-> st X; st)).
    { Search "t_update_shadow".  apply t_update_shadow.   }
    rewrite H1. rewrite t_update_same. assumption.
  - inversion H . subst. inversion H. subst. rewrite t_update_shadow.
    induction a; eauto.
    + simpl. rewrite t_update_same. apply t_update_eq.
    + simpl. rewrite t_update_same. rewrite t_update_eq. auto.
    + simpl. rewrite t_update_same. rewrite t_update_eq. auto.
...
Proof.
  eapply hoare_consequence_pre.
  - simpl. Search "hoare_asgn". apply hoare_asgn.
  - assn_auto. Qed.
...
Proof.
  eapply hoare_consequence_pre.
  - apply hoare_asgn.
  - unfold "->>", assn_sub, t_update. simpl. intros. assumption. Qed.
...
Example hoare_asgn_example3 : forall (a:aexp) (n:nat),
  ...
  {{X = n}}.
Proof.
  intros a n. Search "hoare_seq". eapply hoare_seq.
  - Search "hoare_skip". (* right part of seq *)
    apply hoare_skip.
  - (* left part of seq *)
    eapply hoare_consequence_pre.
    + Search "hoare_asgn". apply hoare_asgn.
    + assn_auto.
Qed.
...
Proof.
  ...
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + unfold assn_sub,t_update. simpl. unfold "->>". intros. reflexivity. Qed.
...
Theorem swap_exercise :
  ...
  {{Y <= X}}.
Proof.
  apply hoare_seq with (Q := (Z = X /\ X <= Y)%assertion).
  - apply hoare_seq with (Q:= (Z <= X)%assertion).
    * eapply hoare_consequence_pre.
      + apply hoare_asgn.
      + unfold "->>", assn_sub,t_update. simpl. intros. assumption.
    * eapply hoare_consequence_pre. 
      + apply hoare_asgn.
      + unfold "->>", assn_sub,t_update. simpl. intros. inversion H.  rewrite <- H0 in H1. assumption.
  - eapply hoare_consequence_pre.
    * apply hoare_asgn.
    * unfold "->>", assn_sub,t_update. simpl. intros. auto. Qed.
...
Proof.
  ...
  simpl in H. 
  specialize H with (st := empty_st) (st' := ( Y !-> (X !->3) X; X !-> 3)).
  simpl in H.
  assert (empty_st =[ X := 3; Y := X ]=> (Y !-> (X !-> 3) X; X !-> 3)).
  { intros. apply E_Seq with (X !-> 3). 
    + apply E_Asgn. simpl. reflexivity.
    + apply E_Asgn. auto. }
  apply H in H0.
  + rewrite t_update_eq in H0. rewrite t_update_eq in H0. inversion H0.
  + apply t_apply_empty. Qed.
...
Proof.
  apply hoare_if; eapply hoare_consequence_pre;
    try apply hoare_asgn; try assn_auto''. Qed.
(** [] *)
...
Theorem hoare_if1 : forall P Q (b:bexp) c,
  {{P /\ b}} c {{Q}} -> {{P /\ ~ b}} (skip) {{Q}} -> {{P}} if1 b then c end {{Q}}.
Proof.
  unfold hoare_triple, bassn. intros. inversion H1; subst.
  ...
  - apply H0 with st'.
    ...
    unfold not. intros. rewrite H3 in H7. discriminate. Qed.
...
Proof.
  ...
  - simpl. eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + assn_auto''.
  - apply hoare_consequence_pre with (fun st : state => Aexp_of_aexp X st = Aexp_of_aexp Z st).
    + unfold hoare_triple. intros. inversion H; subst. assumption.
    + simpl. unfold bassn, assn_sub, update, assert_implies. intros.
      inversion H. apply eq_true_negb_classical in H1. apply beq_nat_true in H1.
      lia. Qed.
...
Proof.
  ...
  - simpl. intros st [Hinv Hguard]. unfold not in Hguard. exfalso. apply Hguard. reflexivity. Qed.
...
Proof.
  unfold havoc_pre, hoare_triple, assn_sub. intros.
  inversion H; subst. simpl in H0. eauto. Qed.
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold "->>", havoc_pre,assn_sub. intros.
    simpl. exists (st X). rewrite t_update_shadow. rewrite t_update_same. assumption. Qed. 
...
Proof.
  exists (fun st => True). 
  exists (BFalse).
  exists (fun st => True). unfold hoare_triple. split.
  - intros. inversion H; subst. exists st. auto.
  - unfold not. intros.
    specialize (H empty_st RError).
    assert (empty_st =[ assert false ]=> RError).
    { apply E_AssertFalse. auto. }
    apply H in H0. 
    + inversion H0. inversion H1. discriminate.
    + auto. Qed.
...
Proof.
  unfold hoare_triple. intros. 
  inversion H.
  -  subst. exists st. split; auto. inversion H0. auto.
  - subst. exists st. split; auto.
    + inversion H0. inversion H0. inversion H3. rewrite H5 in H2. discriminate.
...
Proof.
  ...
  - apply hoare_seq with (fun st => True /\  (st X = 2)).
    + eapply hoare_consequence_pre.
      *  apply hoare_assert.
      * assn_auto''.
    + eapply hoare_consequence_pre.
